...
function showHymn(n,scrollBehavior='smooth'){if(!Number.isInteger(n)||n<1||n>hymns.length)return;clearModes();mode='hymn';current=n;const a=document.getElementById(`hymn-${n}`);a.classList.add('show');history.replaceState(null,'',`#hymn-${n}`);setTimeout(()=>{const title=a.querySelector('h2');const y=title.getBoundingClientRect().top+window.scrollY-8;window.scrollTo({top:y,behavior:scrollBehavior})},0)}
...
el.search.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();doSearch()}});el.search.addEventListener('change',doSearch);
...
render();setZoom(+(localStorage.getItem('rooted-hymn-size')||85));const m=location.hash.match(/^#hymn-(\d+)$/);if(m)showHymn(+m[1]);
})();
