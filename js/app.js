...
function showHymn(n,scrollBehavior='smooth'){if(!Number.isInteger(n)||n<1||n>hymns.length)return;clearModes();mode='hymn';current=n;const a=document.getElementById(`hymn-${n}`);a.classList.add('show');history.replaceState(null,'',`#hymn-${n}`);setTimeout(()=>{updateStandaloneScrollTail(a);const title=a.querySelector('h2');const y=title.getBoundingClientRect().top+window.pageYOffset-8;window.scrollTo({top:y,behavior:scrollBehavior})},30)}
