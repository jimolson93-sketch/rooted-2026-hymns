...
function initTextSearch(){
 ...
 searchModeBtn=document.createElement('button');searchModeBtn.type='button';searchModeBtn.className='search-mode-btn';shell.appendChild(searchModeBtn);
...
el.search.addEventListener('input',()=>{if(ENABLE_TEXT_SEARCH&&searchMode==='text'){renderTextSearchResults(el.search.value);return}if(!el.search.value.trim()&&mode==='hymn')showIdle()});
el.search.addEventListener('keydown',e=>{if(ENABLE_TEXT_SEARCH&&searchMode==='text'){if(e.key==='Enter')e.preventDefault();return}if(e.key==='Enter'){e.preventDefault();suppressNextChange=true;doSearch();setTimeout(()=>{suppressNextChange=false},0)}});
el.search.addEventListener('change',()=>{if(ENABLE_TEXT_SEARCH&&searchMode==='text')return;if(suppressNextChange){suppressNextChange=false;return}doSearch()});
