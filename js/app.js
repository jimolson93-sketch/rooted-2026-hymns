...
function clearModes(){document.body.classList.remove('show-all-mode');el.index.classList.remove('show');document.querySelectorAll('.hymn.show').forEach(x=>x.classList.remove('show'))}
function showIndex(){clearModes();mode='index';current=null;el.index.classList.add('show');window.scrollTo({top:0,behavior:'smooth'})}
function showAll(){clearModes();mode='all';current=null;document.body.classList.add('show-all-mode');window.scrollTo({top:0,behavior:'smooth'})}
